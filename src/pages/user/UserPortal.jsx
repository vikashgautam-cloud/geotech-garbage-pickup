...
import { STATIONS, GARBAGE_TYPES, AREAS, STATUS_META } from '../../data/mockData';
...
const GTYPE_ICONS = {
  'Plastic Waste': Trash2, 'Food Waste': Package,
  'Abandoned Item': Package, 'Chemical Spill': FlaskConical,
  'Recyclable': Recycle, 'Recyclable Waste': Recycle,
  'Sewage / Water': Droplets,
};
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span style={{ background: s.bg, color: s.color, padding: '3px 10px', borderRadius: 99, fontSize: 10.5, fontWeight: 700, whiteSpace: 'nowrap' }}>
      {s.label}
    </span>
  );
};
...
const TopBar = ({ title, onBack, right }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 16px', borderBottom: `1px solid ${T.border}`, background: T.white, flexShrink: 0 }}>
    {onBack && (
      <button onClick={onBack} style={{ width: 34, height: 34, borderRadius: '50%', background: T.surface, border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
...
function BottomNav({ active, onChange }) {
  ...
  return (
    <div style={{ background: T.white, borderTop: `1px solid ${T.border}`, display: 'flex', flexShrink: 0 }}>
      {tabs.map(b => (
...
function MapHome({ onReport, myReports, liveToast, onClearToast, onTabChange }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      {/* Header */}
      <div style={{ background: `linear-gradient(135deg, ${T.navy} 0%, ${T.navyMid} 100%)`, padding: '12px 18px 12px', flexShrink: 0 }}>
      ...
      <div style={{ background: T.white, borderTop: `1px solid ${T.border}`, padding: '10px 14px', maxHeight: 175, overflowY: 'auto', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          ...
          {myReports.length > 0 && (
            <button onClick={() => onTabChange('reports')} style={{ fontSize: 11, color: T.blue, fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3 }}>
              View all <ChevronRight size={12} />
            </button>
          )}
        </div>
        {myReports.length === 0 ? (
          <div style={{ fontSize: 12, color: T.text3, textAlign: 'center', padding: '10px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            <MapPin size={13} color={T.text3} /> Tap the button above to make your first report
          </div>
        ) : (
...
/* ══════════════════════════════════════════
   SCREEN: REPORTS (dynamic from Firestore + local)
══════════════════════════════════════════ */
function ReportsScreen({ myReports, onTabChange }) {
  ...
  // Merge local (just submitted) reports with Firestore data
  // myReports are keyed by their generated SR-xxx id; firestore uses doc id
  ...
  const allReports = [
    ...
    ...(complaints || []).map(c => ({ ...c, _source: 'firestore' })),
  ];
  ...
  const filtered = allReports.filter(r => {
    ...
    const matchSearch = !q || (r.gtype || '').toLowerCase().includes(q) ||
      (r.area || '').toLowerCase().includes(q) ||
      (r.id || '').toLowerCase().includes(q) ||
      (r.customTicketId || '').toLowerCase().includes(q);
    return matchStatus && matchSearch;
  });

  // Stats
  const total = allReports.length;
  const done = allReports.filter(r => r.status === 'COMPLETED').length;
  const pending = allReports.filter(r => ['NEW', 'ACCEPTED', 'IN_PROGRESS'].includes(r.status)).length;
  const escalated = allReports.filter(r => r.status === 'ESCALATED').length;

  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      {/* Header */}
      <div style={{ background: `linear-gradient(135deg, ${T.navy} 0%, ${T.navyMid} 100%)`, padding: '14px 18px 14px', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {[
            { label: 'Total', value: total, color: '#42A5F5' },
            { label: 'Pending', value: pending, color: '#FFA726' },
            { label: 'Done', value: done, color: '#66BB6A' },
            { label: 'Escalated', value: escalated, color: '#EF5350' },
          ].map(s => (
      ...
      <div style={{ display: 'flex', gap: 6, padding: '8px 14px', overflowX: 'auto', flexShrink: 0, background: T.white, borderBottom: `1px solid ${T.border}` }}>
        {FILTERS.map(f => {
          const count = f === 'ALL' ? allReports.length : allReports.filter(r => r.status === f).length;
      ...
      <div style={{ flex: 1, overflowY: 'auto', background: T.surface }}>
        {loading && allReports.length === 0 ? (
          <div style={{ padding: 32, textAlign: 'center', color: T.text3, fontSize: 13 }}>
            <TrendingUp size={28} style={{ display: 'block', margin: '0 auto 8px' }} />
          ...
          <div style={{ padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {filtered.map(r => {
              const Icon = GTYPE_ICONS[r.gtype] || Trash2;
              const ticketId = r.customTicketId || r.id;
              const isOpen = expanded === ticketId;
                ...
                <div key={ticketId} style={{ background: T.white, borderRadius: 12, border: `1px solid ${T.border}`, overflow: 'hidden' }}>
                  ...
                  <div onClick={() => setExpanded(isOpen ? null : ticketId)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 13px', cursor: 'pointer' }}>
                    ...
                    <div style={{ width: 44, height: 44, borderRadius: 8, overflow: 'hidden', flexShrink: 0, background: T.surface, border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {r.image || r.photoURL
                        ? <img src={r.image || r.photoURL} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        : <Icon size={18} color={T.gray} />
                      }
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                        ...
                        {r._source === 'local' && (
                          <span style={{ fontSize: 9, background: T.blueXL, color: T.blue, padding: '1px 5px', borderRadius: 4, fontWeight: 700 }}>LOCAL</span>
                        )}
                      </div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: T.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{r.gtype}</div>
                      <div style={{ fontSize: 11, color: T.text3, display: 'flex', alignItems: 'center', gap: 4, marginTop: 1 }}>
                  ...
                  {isOpen && (
                    <div style={{ borderTop: `1px solid ${T.border}`, padding: '10px 13px', background: T.surface }}>
                      {/* Full photo */}
                      {(r.image || r.photoURL) && (
                        <div style={{ borderRadius: 8, overflow: 'hidden', marginBottom: 10, maxHeight: 160 }}>
                          <img src={r.image || r.photoURL} alt="evidence" style={{ width: '100%', height: 160, objectFit: 'cover', display: 'block' }} />
                        </div>
                      )}
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {[
                          ...
                          ['Area',       r.area || '—'],
                          ['Severity',   r.severity || '—'],
                          ['Reported by', r.reportedBy || 'Citizen (App)'],
                          ['GPS',        r.lat && r.lng ? `${r.lat}°N, ${r.lng}°E` : '—'],
                          ...(r.desc ? [['Note', r.desc]] : []),
                          ...(r.resolvedAt ? [['Resolved', timeAgo(r.resolvedAt)]] : []),
                          ...(r.resolutionMins ? [['Resolution time', `${r.resolutionMins} min`]] : []),
                        ].map(([k, v]) => (
...
/* ══════════════════════════════════════════
   SCREEN: HELP (dynamic — FAQ + contact)
══════════════════════════════════════════ */
function HelpScreen({ onTabChange }) {
  ...
  const avgRes = (() => {
    const resolved = (complaints || []).filter(c => c.resolutionMins);
    if (!resolved.length) return null;
    return Math.round(resolved.reduce((s, c) => s + c.resolutionMins, 0) / resolved.length);
  })();

  const FAQS = [
    {
      q: 'How do I report garbage?',
      a: 'Tap "Report Garbage" on the Map screen, take a photo, select the garbage type, confirm your location and nearest station, then tap "Send Report". Your complaint is instantly forwarded to the assigned cleaning vendor.',
    },
    {
      q: 'Do I need to create an account?',
      a: 'No. Swachh Rail works without any login or registration. Just open the app and report directly. Your GPS coordinates are captured automatically.',
    },
    {
      q: 'How can I track my complaint?',
      a: `Go to the Reports tab to see all submitted complaints with live status updates. ${total > 0 ? `There are currently ${total} reports in the system.` : ''} Each report shows its current status: Pending → Accepted → In Progress → Done.`,
    },
    {
      q: 'How quickly is a complaint resolved?',
      a: avgRes
        ? `Currently, the average resolution time is ${avgRes} minutes. ${completed} out of ${total} reports have been resolved.`
        : 'Most complaints are addressed within 30–90 minutes depending on severity. Critical issues like chemical spills are escalated immediately.',
    },
    {
      q: 'What types of garbage can I report?',
      a: `You can report: ${GARBAGE_TYPES.map(g => g.label).join(', ')}. Chemical spills and sewage issues are treated as critical and escalated immediately.`,
    },
    {
      q: 'Which areas and stations are covered?',
      a: `Currently covering: ${STATIONS.map(s => s.name).join(', ')}. Areas include all platforms, waiting halls, foot overbridges, circulating areas, parking zones, and food courts.`,
    },
    {
      q: 'What happens after I submit a report?',
      a: '1. Your report is logged with GPS and photo evidence.\n2. The assigned vendor is notified instantly.\n3. A cleaner is dispatched to the location.\n4. Status updates in real-time on the Reports screen.\n5. You receive a ticket number to track progress.',
    },
    {
      q: 'My camera or GPS is not working. What should I do?',
      a: 'Make sure you have granted camera and location permissions in your browser/device settings. If GPS fails, the app defaults to Nagpur Railway Station coordinates. You can adjust the station and area manually before submitting.',
    },
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      {/* Header */}
      <div style={{ background: `linear-gradient(135deg, ${T.navy} 0%, ${T.navyMid} 100%)`, padding: '14px 18px', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          <Logo />
          <div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {[
            { label: 'Reports Filed', value: total, icon: FileText },
            { label: 'Resolved', value: completed, icon: CheckCircle2 },
            { label: 'Avg. Resolution', value: avgRes ? `${avgRes}m` : 'N/A', icon: Clock },
          ].map(s => (
            <div key={s.label} style={{ flex: 1, background: 'rgba(255,255,255,.08)', borderRadius: 10, padding: '8px 6px', textAlign: 'center' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', background: T.surface }}>
        ...
        <div style={{ margin: '12px 12px 0', background: T.blueXL, border: `1px solid ${T.blue}33`, borderRadius: 12, padding: '12px 14px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}>
            <Info size={15} color={T.blue} />
            <span style={{ fontSize: 13, fontWeight: 700, color: T.blue }}>How to Use</span>
          </div>
          {[
            { step: '1', text: 'Open the Map tab and tap "Report Garbage"' },
            { step: '2', text: 'Take a photo of the garbage or issue' },
            { step: '3', text: 'Select the garbage type from the list' },
            { step: '4', text: 'Confirm your location and nearest station' },
            { step: '5', text: 'Tap "Send Report" — done! Vendor is notified' },
          ].map(s => (
        ...
        <div style={{ padding: '14px 12px 6px' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: T.text, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            <Phone size={13} color={T.text} /> Contact
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {CONTACTS.map(c => (
              <div key={c.label} style={{ background: T.white, border: `1px solid ${T.border}`, borderRadius: 11, padding: '11px 13px', display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: T.blueXL, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <c.icon size={16} color={T.blue} />
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <a href={c.isEmail ? `mailto:${c.name}` : `tel:${c.phone}`} style={{ width: 32, height: 32, borderRadius: '50%', background: T.blue, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, textDecoration: 'none' }}>
                  <c.icon size={14} color={T.white} />
                </a>
              </div>
            ))}
          </div>
        ...
        <div style={{ padding: '14px 12px 12px' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: T.text, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            <MessageCircle size={13} color={T.text} /> Frequently Asked Questions
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {FAQS.map((faq, i) => {
              const isOpen = openFaq === i;
              return (
                <div key={i} style={{ background: T.white, border: `1px solid ${isOpen ? T.blue : T.border}`, borderRadius: 11, overflow: 'hidden', transition: 'border-color .2s' }}>
                  ...
                  {isOpen && (
                    <div style={{ padding: '0 13px 13px', fontSize: 12.5, color: T.text2, lineHeight: 1.7, whiteSpace: 'pre-line' }}>
                      {faq.a}
                    </div>
                  )}
                </div>
        ...
        {/* Garbage types reference */}
        <div style={{ padding: '0 12px 16px' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: T.text, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            <AlertCircle size={13} color={T.text} /> Severity Guide
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {GARBAGE_TYPES.map(g => {
              ...
              return (
                <div key={g.id} style={{ background: T.white, border: `1px solid ${T.border}`, borderRadius: 10, padding: '9px 13px', display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ background: T.grayL, padding: 7, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon size={16} color={T.gray} />
                  </div>
                  <span style={{ flex: 1, fontSize: 13, fontWeight: 600, color: T.text }}>{g.label}</span>
                  <span style={{ fontSize: 10, fontWeight: 700, background: sevBg, color: sevColor, padding: '3px 8px', borderRadius: 99, textTransform: 'uppercase' }}>
                    {g.severity}
                  </span>
                </div>
              );
...
function CameraScreen({ onCapture, onBack }) {
  ...
  const [facing, setFacing] = useState(window.location.hostname === 'localhost' ? 'user' : 'environment');
  ...
  const startCamera = useCallback(async (facingMode) => {
    ...
    try {
      ...
      const constraints = window.location.hostname === 'localhost'
        ? { video: { width: { ideal: 640 }, height: { ideal: 480 } }, audio: false }
        : { video: { facingMode, width: { ideal: 1280 }, height: { ideal: 720 } }, audio: false };
      const stream = await navigator.mediaDevices.getUserMedia(constraints);
      if (activeRequestRef.current !== facingMode) { stream.getTracks().forEach(t => t.stop()); return; }
      streamRef.current = stream;
      if (videoRef.current) {
        videoRef.current.srcObject = stream;
        videoRef.current.setAttribute('playsinline', true);
        await videoRef.current.play().catch(err => { if (err.name !== 'AbortError') throw err; });
        setCamReady(true); setCamErr(null);
      }
    } catch (e) {
      if (facingMode === 'environment') setFacing('user');
      else setCamErr('Camera hardware not responding or locked by another application.');
    }
  }, []);

  useEffect(() => {
    startCamera(facing);
    return () => { activeRequestRef.current = null; if (streamRef.current) { streamRef.current.getTracks().forEach(t => t.stop()); streamRef.current = null; } };
  }, [facing, startCamera]);
  ...
  const flipCamera = () => { if (!camReady) return; setCamReady(false); setFacing(f => f === 'environment' ? 'user' : 'environment'); };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, background: '#000', overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10, background: 'linear-gradient(to bottom,rgba(0,0,0,.65),transparent)' }}>
        <button onClick={onBack} style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(0,0,0,.45)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><X size={18} color={T.white} /></button>
        ...
        <button onClick={flipCamera} style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(0,0,0,.45)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><RotateCcw size={16} color={T.white} /></button>
      </div>
      ...
      <div style={{ background: '#111', padding: '18px 24px 26px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <div style={{ width: 36 }} />
        <button onClick={shoot} disabled={!camReady || !gps} style={{ width: 72, height: 72, borderRadius: '50%', background: camReady && gps ? T.white : '#555', border: '4px solid rgba(255,255,255,.3)', cursor: camReady && gps ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ height: 110, background: '#000', flexShrink: 0, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'rgba(0,0,0,.6)', color: T.white, padding: '5px 13px', borderRadius: 99, fontSize: 11, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '8px 16px', overflowY: 'auto', flex: 1 }}>
        {GARBAGE_TYPES.map(g => {
          ...
          return (
            <div key={g.id} onClick={() => setSel(g.id)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: isSelected ? T.blueXL : T.surface, border: `1.5px solid ${isSelected ? T.blue : T.border}`, borderRadius: 12, padding: '10px 14px', cursor: 'pointer', transition: 'all .2s ease' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ background: isSelected ? T.white : T.grayL, padding: 7, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Icon size={18} color={isSelected ? T.blue : T.gray} />
                </div>
      ...
      <div style={{ padding: '12px 16px 16px', flexShrink: 0, background: T.white, borderTop: `1px solid ${T.border}` }}>
        <button disabled={!sel} onClick={() => onSelect(GARBAGE_TYPES.find(g => g.id === sel))} style={{ width: '100%', background: sel ? T.blue : T.grayL, color: sel ? T.white : T.gray, padding: 13, borderRadius: 12, fontSize: 14.5, fontWeight: 800, border: 'none', cursor: sel ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        <div style={{ padding: '13px 15px' }}>
          ...
          <div style={{ marginBottom: 10 }}>
            <label style={{ display: 'block', fontSize: 10.5, fontWeight: 700, color: T.text3, textTransform: 'uppercase', letterSpacing: .5, marginBottom: 5 }}>Nearest Station</label>
            <select value={station} onChange={e => setStation(e.target.value)} style={{ width: '100%', padding: '9px 11px', border: `1px solid ${T.border}`, borderRadius: 9, fontSize: 13, fontFamily: 'inherit', background: T.white }}>
              {STATIONS.map(s => <option key={s.id} value={s.name}>{s.name}</option>)}
            </select>
          </div>
          <div style={{ marginBottom: 10 }}>
            <label style={{ display: 'block', fontSize: 10.5, fontWeight: 700, color: T.text3, textTransform: 'uppercase', letterSpacing: .5, marginBottom: 5 }}>Exact Area</label>
            <select value={area} onChange={e => setArea(e.target.value)} style={{ width: '100%', padding: '9px 11px', border: `1px solid ${T.border}`, borderRadius: 9, fontSize: 13, fontFamily: 'inherit', background: T.white }}>
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            <textarea value={note} onChange={e => setNote(e.target.value)} maxLength={200} placeholder="Describe what you see…" style={{ width: '100%', padding: '9px 11px', border: `1px solid ${T.border}`, borderRadius: 9, fontSize: 13, minHeight: 56, resize: 'none', fontFamily: 'inherit' }} />
          </div>
        </div>
      </div>
      <div style={{ padding: '11px 14px 15px', borderTop: `1px solid ${T.border}`, display: 'flex', gap: 9, flexShrink: 0, background: T.white }}>
        ...
        <button onClick={() => onSend({ gtype, station, area, note, lat: gps?.lat, lng: gps?.lng, photoURL })} disabled={isSubmitting} style={{ flex: 1, background: isSubmitting ? T.gray : T.blue, color: T.white, padding: 12, borderRadius: 11, fontSize: 14, fontWeight: 800, border: 'none', cursor: isSubmitting ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, boxShadow: `0 2px 12px ${T.blue}55` }}>
...
function SuccessScreen({ ticket, photoURL, gps, gtype, station, onDone }) {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '11px 16px 18px', borderTop: `1px solid ${T.border}`, flexShrink: 0, background: T.white }}>
        <button onClick={onDone} style={{ width: '100%', background: T.blue, color: T.white, padding: 13, borderRadius: 12, fontSize: 14, fontWeight: 700, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
...
export default function UserPortal() {
  ...
  const [tab, setTab] = useState('map');           // 'map' | 'reports' | 'help'
  const [screen, setScreen] = useState('home');    // 'home' | 'camera' | 'picker' | 'confirm' | 'success'
  ...
  const handleSend = async (data) => {
    ...
    try {
      ...
      await addComplaint({
        gtype: data.gtype.label, severity: data.gtype.severity,
        station: data.station, area: data.area, note: data.note,
        lat: data.lat, lng: data.lng, photoURL: data.photoURL,
      });
      ...
      setSentData(data);
      setMyReports(prev => [{
        id: finalTicketId, gtype: data.gtype.label, area: data.area,
        photoURL: data.photoURL, station: { name: data.station },
        status: 'NEW', reportedAt: new Date(),
  ...
  // Render tab-based screens (map/reports/help) vs flow screens (camera/picker/confirm/success)
  const renderContent = () => {
    // Flow screens override tab
  ...
  return (
    <div style={{ width: '100%', maxWidth: 430, margin: '0 auto', background: T.white, display: 'flex', flexDirection: 'column', height: '100vh', fontFamily: 'DM Sans, system-ui, sans-serif', overflow: 'hidden' }}>
      {renderContent()}
    </div>
  );
}
