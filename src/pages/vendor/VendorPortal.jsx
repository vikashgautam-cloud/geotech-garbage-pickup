...
const Btn = ({children,onClick,color,outline=false,small=false,type='button',icon,t})=>{
  ...
  return (
    <button type={type} onClick={onClick} style={{padding:small?'5px 11px':'8px 14px',borderRadius:5,fontSize:small?11:12,fontWeight:600,cursor:'pointer',background:outline?'transparent':bg,color:outline?bg:'#fff',border:outline?`1px solid ${bg}`:'none',display:'inline-flex',alignItems:'center',gap:5,fontFamily:'IBM Plex Sans, sans-serif',letterSpacing:'.02em'}}>
...
function ImageLightbox({imageUrl,onClose}) {
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(4,8,20,0.96)',zIndex:99999,display:'flex',alignItems:'center',justifyContent:'center',cursor:'zoom-out',backdropFilter:'blur(4px)'}}>
      <div style={{position:'relative',maxWidth:'96vw',maxHeight:'96vh',display:'flex',flexDirection:'column',alignItems:'center'}} onClick={e=>e.stopPropagation()}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',width:'100%',padding:'0 0 10px',gap:10}}>
          <span style={{fontSize:11,color:'rgba(255,255,255,0.4)',fontFamily:'IBM Plex Mono,monospace',letterSpacing:'.04em',textTransform:'uppercase'}}>Evidence Photo</span>
          <div style={{display:'flex',gap:8}}>
            <a href={imageUrl} download target="_blank" rel="noreferrer"
              style={{background:'rgba(255,255,255,0.1)',border:'1px solid rgba(255,255,255,0.15)',color:'#fff',cursor:'pointer',width:32,height:32,borderRadius:5,display:'flex',alignItems:'center',justifyContent:'center',textDecoration:'none'}} aria-label="Download">
...
/* ── Clickable Image ── */
const ClickableImg = ({src,alt='',style={},onViewImage})=>(
  <img src={src} alt={alt}
    onClick={e=>{e.stopPropagation();onViewImage(src);}}
...
function TopHeader({t,dark,onToggleDark,vendorData,onLogout,onHamburger}) {
  return (
    <div style={{background:t.headerBg,padding:'0 16px',display:'flex',alignItems:'center',justifyContent:'space-between',height:52,flexShrink:0,borderBottom:`1px solid ${dark?'rgba(255,255,255,0.06)':'rgba(0,0,0,0.12)'}`}}>
      <div style={{display:'flex',alignItems:'center',gap:8}}>
        <button onClick={onHamburger} className="hamburger-btn"
        ...
        <div>
          <div style={{fontSize:13,fontWeight:700,color:'#fff',letterSpacing:'-.01em',lineHeight:1}}>Vendor Login</div>
          <div style={{fontSize:9,color:'rgba(255,255,255,0.4)',fontFamily:'IBM Plex Mono,monospace',letterSpacing:'.05em',marginTop:2}}>{vendorData?.stationName||'NAGPUR JUNCTION · CENTRAL RAILWAY'}</div>
        </div>
      </div>
      <div style={{display:'flex',alignItems:'center',gap:8}}>
        <button onClick={onToggleDark}
          ...
          <span className="hide-xs">{dark?'Light':'Dark'}</span>
        </button>
        {vendorData&&(
        ...
        <button onClick={onLogout}
          ...
          <span className="hide-xs">Exit</span>
        </button>
      </div>
    </div>
  );
...
const SBadge = ({status})=>{
  ...
  return <span style={{background:m.bg,color:m.color,padding:'3px 9px',borderRadius:99,fontSize:10,fontWeight:700,whiteSpace:'nowrap'}}>{m.label}</span>;
};
      ...
      <div style={{padding:'11px 13px',flex:1,display:'flex',flexDirection:'column',gap:8}}>
        ...
        {c.status==='ASSIGNED_TO_CLEANER'&&(
          <div style={{fontSize:11,color:t.amber,fontWeight:600,background:t.amberLight,padding:'6px 10px',borderRadius:5,display:'flex',alignItems:'center',gap:5}}>
            <i className="ti ti-user-check" style={{fontSize:13}} aria-hidden="true"/>On Duty: {c.assignedTo||'Cleaner'}
          </div>
        )}
...
/* ── Cleaner Dashboard ── */
function CleanerDashboard({cleanerData,vendorId,tasks,onUpdateComplaint,onLogout,onViewImage,t}) {
        ...
        : <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {myTasks.map(task=>(
            <div key={task.id} style={{background:t.surface,padding:13,borderRadius:8,border:`1px solid ${t.border}`}}>
              {(task.image||task.photoURL)&&(
                <ClickableImg src={task.image||task.photoURL} alt="Report" onViewImage={onViewImage}
                  style={{width:'100%',height:100,objectFit:'cover',borderRadius:6,marginBottom:10,display:'block'}}/>
              )}
...
export default function VendorPortal() {
  ...
  const handleLogout=()=>{setIsAuthenticated(false);setCurrentVendorData(null);setVendorLoginId('');setVendorPassword('');};
  ...
  const responsiveCss = `
    ...
    * { box-sizing: border-box; }
    ...
    .vendor-sidebar-overlay { display: none; }
      ...
      <div style={{display:'flex',minHeight:'100vh',flexDirection:'column',background:dark?'#030810':'#F0F1F3',fontFamily:'IBM Plex Sans,sans-serif'}}>
        ...
        <div style={{
          ...
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            ...
            <div>
              <div style={{fontSize:13,fontWeight:700,color:'#fff',letterSpacing:'-.01em',lineHeight:1}}>
                Railway Material Management System
              </div>
              <div style={{fontSize:9,color:'rgba(255,255,255,0.45)',fontFamily:'IBM Plex Mono,monospace',letterSpacing:'.05em',marginTop:2}}>
                CENTRAL RAILWAY · NAGPUR DIVISION
              </div>
            </div>
          </div>

          {/* Right: dark toggle + portal badge */}
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            ...
            <div style={{
              ...
              fontFamily:'IBM Plex Sans,sans-serif',letterSpacing:'.04em',textTransform:'uppercase',
            }}>
              VENDOR PORTAL
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:20,paddingBottom:16,borderBottom:`1px solid ${t.border}`}}>
              ...
              <div>
                ...
                <div style={{fontSize:10,color:t.text3,fontFamily:'IBM Plex Mono,monospace',marginTop:3,letterSpacing:'.04em'}}>
                  NAGPUR JUNCTION · CENTRAL RAILWAY
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleVendorAuth}>
              <label style={{display:'block',fontSize:10,fontWeight:600,color:t.text3,marginBottom:5,textTransform:'uppercase',letterSpacing:'.07em',fontFamily:'IBM Plex Mono,monospace'}}>
                Vendor Station ID
              </label>
              <input
                type="text"
                value={vendorLoginId}
                onChange={e=>setVendorLoginId(e.target.value)}
                placeholder="e.g. platform_no_1"
                style={{width:'100%',padding:'10px 12px',borderRadius:6,border:`1px solid ${t.border}`,marginBottom:14,fontSize:13,fontFamily:'IBM Plex Sans,sans-serif',background:t.surface2,color:t.text,outline:'none'}}
                required
              />
              <label style={{display:'block',fontSize:10,fontWeight:600,color:t.text3,marginBottom:5,textTransform:'uppercase',letterSpacing:'.07em',fontFamily:'IBM Plex Mono,monospace'}}>
                Access Password
              ...
              <input
                ...
                placeholder="••••••••"
                style={{width:'100%',padding:'10px 12px',borderRadius:6,border:`1px solid ${t.border}`,marginBottom:20,fontSize:13,fontFamily:'IBM Plex Sans,sans-serif',background:t.surface2,color:t.text,outline:'none'}}
                required
              />
              <button
                type="submit"
            ...
            <div style={{
              ...
              <div style={{fontSize:11,color:t.text3,lineHeight:1.5}}>
                Credentials are issued by your department supervisor. Contact them if you need access.
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        display:'flex',
        flex:1,
        minHeight:0,      /* ← critical: lets children shrink below content size */
        overflow:'hidden',
        position:'relative',
      }}>
        {/* Mobile overlay */}
        {sidebarOpen&&(
          <div className="vendor-sidebar-overlay" onClick={()=>setSidebarOpen(false)}/>
        )}
          ...
          <div style={{padding:20,maxWidth:880,margin:'0 auto',width:'100%',boxSizing:'border-box'}}>
            ...
            {tab==='map'&&(
              <>
                ...
                <LiveMap
                  complaints={filteredComplaints.filter(c=>c.lat&&c.lng)}
                  height="400px"
              ...
              <>
                <div style={{marginBottom:20}}>
                  ...
                  <div style={{fontSize:11,color:t.text3,marginTop:2}}>
                    {filteredComplaints.filter(c=>c.status==='COMPLETED').length} resolved tasks
                  </div>
                </div>
