// CleanerPortal.jsx  ← full file, drop-in replacement
// Only handleLogin and the Top Bar section are changed.
// Everything else — themes, CSS, icons, camera, lightbox,
// login screen, layout, task list, detail panel — is identical.
...
const I = {
  ...
  Zone:    () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/><line x1="12" y1="12" x2="12" y2="16"/><line x1="10" y1="14" x2="14" y2="14"/></svg>,
};

/* ── Themes ── (unchanged) */
const LIGHT = {
  bg:'#F2F4F7', surface:'#FFFFFF', surface2:'#F8F9FB', surface3:'#EEF1F6',
  border:'#DDE1EA', accent:'#1A3FBF', green:'#0E7A5A', amber:'#92400E', red:'#B91C1C',
  text:'#0C1A35', text2:'#3D4F72', text3:'#7A8BAA',
  cardShadow:'0 1px 3px rgba(0,0,0,0.06)', headerBg:'#1A3FBF',
  sidebarBg:'#FFFFFF',
};
const DARK = {
  bg:'#080E1A', surface:'#0D1526', surface2:'#111E30', surface3:'#162235',
  border:'rgba(255,255,255,0.07)', accent:'#3B82F6', green:'#10B981', amber:'#F59E0B', red:'#EF4444',
  text:'#E6EDF8', text2:'#8FA3C4', text3:'#4A5F80',
  cardShadow:'none', headerBg:'#060D1A',
  sidebarBg:'#0D1526',
};
...
.cp-sidebar-wrap{width:220px;flex-shrink:0;height:100%;display:flex;flex-direction:column;background:${t.sidebarBg};border-right:1px solid ${t.border};}
...
.cp-topbar-left{display:flex;align-items:center;gap:10px;flex-wrap:wrap;}
...
.cp-zone-pill{display:flex;align-items:center;gap:5px;background:rgba(26,63,191,0.07);border:1px solid rgba(26,63,191,0.18);border-radius:5px;padding:4px 10px;}
.cp-zone-label{font-size:9px;color:${t.text3};font-family:'DM Mono',monospace;letter-spacing:.3px;text-transform:uppercase;}
.cp-zone-val{font-size:10px;font-weight:700;color:${t.accent};font-family:'DM Mono',monospace;text-transform:uppercase;}
.cp-zone-divider{width:1px;height:10px;background:rgba(26,63,191,0.25);display:inline-block;}
.cp-topbar-right{display:flex;align-items:center;gap:8px;}
.cp-icon-btn{background:${t.surface2};border:1px solid ${t.border};border-radius:5px;padding:5px 9px;cursor:pointer;display:flex;align-items:center;gap:5px;font-size:10px;font-weight:600;color:${t.text3};font-family:'DM Sans',sans-serif;}
...
.cp-photo-wrap img{width:100%;display:block;max-height:180px;object-fit:cover;cursor:zoom-in;}
...
.cp-lb{position:fixed;inset:0;background:rgba(0,0,0,0.94);display:flex;align-items:center;justify-content:center;z-index:99999;padding:20px;cursor:zoom-out;backdrop-filter:blur(4px);}
.cp-lb-inner{position:relative;max-width:96vw;max-height:96vh;display:flex;flex-direction:column;align-items:center;}
.cp-lb img{max-width:100%;max-height:calc(96vh - 60px);border-radius:8px;object-fit:contain;display:block;}
.cp-lb-bar{display:flex;align-items:center;justify-content:space-between;width:100%;padding:0 0 10px;gap:10px;}
.cp-lb-label{font-size:11px;color:rgba(255,255,255,0.4);font-family:'DM Mono',monospace;letter-spacing:.04em;text-transform:uppercase;}
.cp-lb-close{background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.15);border-radius:50%;width:36px;height:36px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#fff;}
.cp-lb-hint{font-size:10px;color:rgba(255,255,255,0.25);margin-top:8px;font-family:'DM Mono',monospace;}
@media(max-width:768px){
  .cp-sidebar-wrap{display:none;}
  .cp-left{width:100%;max-width:100%;}
  .cp-split{flex-direction:column;}
}
`;

/* ── Full Screen Camera (unchanged) ── */
function FullScreenCamera({ onCapture, onClose }) {
  ...
  return (
    <div className="cp-fs-cam">
      ...
      {flash && <div style={{ position:'absolute', inset:0, background:'#fff', opacity:.85, zIndex:2, pointerEvents:'none' }} />}
      ...
      <div className="cp-fs-cam-controls">
        ...
        <button className="cp-fs-shoot" onClick={shoot} disabled={!ready}>
          <div style={{ width:52, height:52, borderRadius:'50%', background:ready?'#1A3FBF':'#444', display:'flex', alignItems:'center', justifyContent:'center' }}>
            <I.Camera />
          </div>
        </button>
...
function Lightbox({ src, label, onClose }) {
  ...
  return (
    <div className="cp-lb" onClick={onClose}>
      <div className="cp-lb-inner" onClick={e => e.stopPropagation()}>
        <div className="cp-lb-bar">
          <span className="cp-lb-label">{label || 'Photo'}</span>
          <div style={{ display:'flex', gap:8 }}>
            <a href={src} download target="_blank" rel="noreferrer"
              style={{ background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.15)', color:'#fff', width:32, height:32, borderRadius:5, display:'flex', alignItems:'center', justifyContent:'center', textDecoration:'none', fontSize:14 }}>
              ↓
            </a>
            <button className="cp-lb-close" onClick={onClose}><I.X /></button>
          </div>
        </div>
        <img src={src} alt={label || 'Photo'} />
        <div className="cp-lb-hint">Click outside or press ESC to close</div>
      </div>
    </div>
  );
}
...
function LoginScreen({ onAuth }) {
  ...
  // ─────────────────────────────────────────────────────────────
  // CHANGED: after finding the cleaner, also grab stationName,
  // areaName, platformNo — first from the cleaner doc itself
  // (set by new handleAddCleaner), then fall back to the parent
  // vendor doc for legacy records that pre-date the new flow.
  // ─────────────────────────────────────────────────────────────
  const handleLogin = async e => {
    ...
    try {
      ...
      let found = null, stationId = '', vendorDocId = '';
      ...
      for (const vDoc of rootSnap.docs) {
        ...
        subSnap.forEach(cDoc => {
          ...
          if (dbId === searchId && dbPw === searchPw) {
            found       = { ...d, _cleanerId: cDoc.id };
            stationId   = vDoc.id;
            vendorDocId = vDoc.id;
          }
        });
      ...
      if (!found) {
        setErr('Invalid ID or password. Please check and try again.');
        setLoading(false);
        return;
      }

      // Resolve station & platform metadata.
      // Priority: fields on the cleaner doc (set by new handleAddCleaner)
      // → fallback: parent vendor document
      let stationName = found.stationName || '';
      let areaName    = found.areaName    || '';
      let platformNo  = found.platformNo  || '';

      if (!stationName || !areaName) {
        try {
          const vendorSnap = await getDoc(doc(db, 'vendors', vendorDocId));
          if (vendorSnap.exists()) {
            const vd = vendorSnap.data();
            stationName = stationName || vd.stationName || '';
            areaName    = areaName    || vd.areaName    || '';
            platformNo  = platformNo  || vd.platformNo  || '';
          }
        } catch (fetchErr) {
          console.warn('Could not fetch vendor doc for zone enrichment:', fetchErr);
        }
      }

      onAuth(
        { ...found, stationName, areaName, platformNo },
        stationId
      );
    } catch (ex) {
      console.error(ex);
  ...
  return (
    <>
      ...
      <div className="cp-login-root">
        <button onClick={()=>setDark(v=>!v)} className="cp-icon-btn"
        ...
        <div className="cp-login-card">
          <div className="cp-login-header">
            <div className="cp-login-avatar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
  <img 
    src="/logo.jpeg" 
    alt="GRWMS Logo" 
  ...
  />
            ...
            <div className="cp-login-title">Cleaner Portal</div>
            <div className="cp-login-sub">GRWMS · Cleaner Portal</div>
          </div>
          {err && <div className="cp-alert err"><I.Alert /><span>{err}</span></div>}
          <form onSubmit={handleLogin}>
            <div className="cp-field">
              <label className="cp-flbl">Cleaner Login ID</label>
              <input className="cp-inp" type="text" value={loginId} onChange={e=>setLoginId(e.target.value)} placeholder="Phone number or login ID" required />
            </div>
            <div className="cp-field">
              <label className="cp-flbl">Password</label>
              <input className="cp-inp" type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••" required />
            </div>
            <button type="submit" className="cp-submit" disabled={loading}
              style={{ background: loading ? t.surface3 : t.accent, marginTop:6 }}>
              {loading ? <><div className="cp-spin" />Authenticating…</> : <><I.Check />Sign In &amp; Sync Work Orders</>}
            </button>
          </form>
        </div>
      </div>
...
export default function CleanerPortal() {
  ...
  const handleAuth    = (data, station) => { setCleanerData(data); setStationId(station); setIsAuthenticated(true); };
  const handleLogout  = () => { setIsAuthenticated(false); setCleanerData(null); setStationId(''); setSelected(null); setCapturedPhoto(null); };
  const handleSelect  = task => { setSelected(task); setCapturedPhoto(null); };
  ...
  if (!isAuthenticated) return <LoginScreen onAuth={handleAuth} />;

  const isDone     = selected?.status === 'CLEANED_BY_FORCE';
  const citizenImg = selected ? getImg(selected) : null;

  // ── Build the zone display string ──────────────────────────
  // e.g. "NAGPUR JUNCTION — PLATFORM 1"  or just "NAGPUR JUNCTION"
  const zoneStation  = (cleanerData?.stationName || stationId || '').toUpperCase();
  const zonePlatform = cleanerData?.platformNo
    ? `PLATFORM ${cleanerData.platformNo}`
    : cleanerData?.areaName
      ? cleanerData.areaName.toUpperCase()
      : '';
  const zoneLabel    = [zoneStation, zonePlatform].filter(Boolean).join(' — ');
      ...
      <div className="cp-root">
        ...
        <div className="cp-sidebar-wrap">
          <Sidebar
            ...
            footerUser={{
              initials:    initials(cleanerData?.name),
              name:        cleanerData?.name || 'Cleaner',
              role:        stationId || 'Field Force',
        ...
        <div className="cp-main">
          ...
          <div className="cp-topbar">
            <div className="cp-topbar-left">
              ...
              {/* Existing: vendor/station ID pill */}
              <div className="cp-station-pill">
                <span className="cp-station-dot" />
                <span className="cp-station-label">Vendor:&nbsp;</span>
                <span className="cp-station-val">{stationId}</span>
              </div>

              {/* ── NEW: Operational Zone pill ── */}
              {/* Shows automatically once stationName/areaName/platformNo
                  are resolved during login. Hidden if no data is available. */}
              {zoneLabel && (
                <div className="cp-zone-pill">
                  <I.Zone />
                  <span className="cp-zone-label">Zone:&nbsp;</span>
                  <span className="cp-zone-val">{zoneLabel}</span>
                </div>
              )}
              ...
              <div className="cp-card" style={{ flex:1 }}>
                ...
                {activeTasks.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      <div className="cp-task-ico" style={{ background: done ? 'rgba(146,64,14,0.1)' : 'rgba(26,63,191,0.1)', color: done ? t.amber : t.accent }}>
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div className="cp-task-sub">Ref: {task.customTicketId || task.id?.substring(0,8)}</div>
            ...
            <div className="cp-right">
              {!selected ? (
                <div className="cp-empty-state">
                  ...
                  <div className="cp-empty-state-text">Select a job from the list<br />to view details and submit proof.</div>
                </div>
              ) : (
                <div className="cp-right-inner">
                  ...
                  <div>
                    ...
                    <div style={{ borderRadius:8, overflow:'hidden', border:`1px solid ${t.border}` }}>
                      <LiveMap singlePin singleCoords={{ lat:selected.lat, lng:selected.lng }} height="140px" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    {citizenImg ? (
                      <div
                        style={{ position:'relative', borderRadius:8, overflow:'hidden', border:`1px solid ${t.border}`, cursor:'zoom-in' }}
                        onClick={() => openLightbox(citizenImg, 'Citizen Report — Before Clean')}
                      >
                        <img
                          src={citizenImg}
                          alt="Citizen report"
                  ...
                  <div style={{ marginTop:'auto' }}>
                    ...
                    {isDone && !capturedPhoto && (
                      <div className="cp-alert warn" style={{ marginBottom:10 }}>
                        ...
                        <div>
                          ...
                          <div>Photo already submitted. Retake and resubmit if needed.</div>
                        </div>
                      </div>
                    )}
                    ...
                    {capturedPhoto && (
                      <div className="cp-photo-wrap">
                        <img
                          src={capturedPhoto}
                          alt="After clean proof"
                          onClick={() => openLightbox(capturedPhoto, 'After-Clean Proof')}
                        />
                        <div className="cp-photo-tap" onClick={() => openLightbox(capturedPhoto, 'After-Clean Proof')}>
                          <I.Eye />View full
                        </div>
                      </div>
                    )}

                    <button className="cp-submit" onClick={handleSubmitProof} disabled={!capturedPhoto || uploading}
